namespace EddiDataDefinitions
{
    public class SignalSource : ResourceBasedLocalizedEDName<SignalSource>
    {
        ...
        private SignalSource (string edname) : base(edname, edname.Replace("$", "").Replace(";", ""))
        { }

        new public static SignalSource FromEDName(string edname)
        {
            if (edname.Contains("$"))
            {
                return ResourceBasedLocalizedEDName<SignalSource>.FromEDName(edname.Replace("$", "").Replace(";", ""));
            }

            return new SignalSource(edname);
        }
    }
}
