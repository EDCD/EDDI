    ...
    public class StarSystemSqLiteRepository : SqLiteBaseRepository, StarSystemRepository
    {
        ...
        public StarSystem GetOrCreateStarSystem(string name, bool fetchIfMissing = true)
        {
            ...
            if (system == null)
            {
                if (fetchIfMissing)
                {
                    system = DataProviderService.GetEdsmFullSystemData(name);
                }
                if (system == null)
                {
        ...
        public StarSystem GetStarSystem(string name, bool refreshIfOutdated = true)
        {
                        ...
                        using (SQLiteDataReader rdr = cmd.ExecuteReader())
                        {
                            if (rdr.Read())
                            {
                                ...
                                if (result != null)
                                {
                                    ...
                                    {
                                        // Data is stale
                                        StarSystem updatedResult = DataProviderService.GetEdsmFullSystemData(name);
            ...
            catch (Exception)
            {
                ...
                try
                {
                    result = DataProviderService.GetEdsmFullSystemData(name);
                    updateStarSystem(result);
                }
                catch (Exception ex)
                {
