...
{
    [PublicAPI]
    public class ShipLockerEvent ( DateTime timestamp, List<MicroResourceAmount> inventory ) : Event( timestamp, NAME )
    {
        ...
        [PublicAPI("The items stored in your ship's micro-resource materials locker (as a list of objects with `name`, `category`, and `amount` properties)")]
        public List<MicroResourceAmount> inventory { get; } = inventory;
    }
}