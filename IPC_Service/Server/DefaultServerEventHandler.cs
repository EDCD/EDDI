...
namespace EddiIPC_Service.Server
{
    ...
    public class DefaultServerEventHandler : IServerEventHandler
    {
        ...
        public async Task HandleConnectAsync ( MessageEnvelope message, ConnectionContext context )
        {
            ...
            try
            {
                if ( !TryGetMessageData<ConnectData>( message.Data, out _ ) )
                {
        ...
        private async Task HandleSetResponderModeAsync( CommandData cmdData, string messageId, ConnectionContext context )
        {
            try
            {
                ...
                if ( cmdData.Parameters?.TryGetValue( "voiceAttackVersion", out var versionObj ) ?? false )
                {
                    switch ( versionObj )
                    {
                        case System.Version parsedVersion:
                            voiceAttackVersion = parsedVersion;
                            break;
                        case string versionText when System.Version.TryParse( versionText, out var parsedVersionValue ):
                            voiceAttackVersion = parsedVersionValue;
                            break;
                    }
                }
                ...
                await handler( enable, voiceAttackVersion, CancellationToken.None ).ConfigureAwait( false );
        ...
        private static bool TryGetMessageData<T> ( object data, out T? typedData ) where T : class
        {
            typedData = data switch
            {
                T exactType => exactType,
                JObject json => json.ToObject<T>(),
                IDictionary<string, object> dictionary => JObject.FromObject( dictionary ).ToObject<T>(),
                _ => null
            };
