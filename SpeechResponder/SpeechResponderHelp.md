...
Variables made available for scripting are in-app when editing a script via the `Variables` button. For event scripts, event-specific variables appear first under the `event` object. Other variables follow below, and event variables are documented in the wiki at https://github.com/EDCD/EDDI/wiki/Events.
