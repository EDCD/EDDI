...
using EddiSpeechService.SpeechEffects.Chorus;
...
using System.Linq;
using Utilities;
using Constants = EddiSpeechService.SpeechEffects.Chorus.Constants;
using Functions = EddiSpeechService.SpeechEffects.Chorus.Functions;
...
internal class ChorusSampleProvider : EffectSampleProvider
{
    ...
    private void InitializeChorusVoices ( int sampleRate, int voiceCount, float[] baseDelaysMs )
    {
        ...
        for ( var v = 0; v < voiceCount; v++ )
        {
            ...
            var posScale = Constants.DepthPosMin + ( ( Constants.DepthPosMax - Constants.DepthPosMin ) * Math.Abs( pos ) );
            ...
            if ( ghostWeightFactor > 0.0001f && v % 2 == 0 && v < Constants.VoiceCount - 1 )
            {
                ...
                var ghostVoice = new ChorusVoice( sampleRate, baseMs + Constants.GhostBaseDelayOffsetMs, ghostDepthMs, ghostFreq, _fxLevel, v )
                {
                    LfoPhase = stagger + Constants.GhostVoicePhaseOffset
                };
